...
const features = [
  {
    ...
    description: "Conteúdo, comunidades e criadores reunidos em uma experiência rápida e personalizada.",
  },
  {
    icon: Users,
    title: "Comunidades de verdade",
    description: "Crie espaços públicos ou privados e fortaleça conexões em torno do que importa.",
  },
  {
    icon: MessageCircle,
    title: "Conversas sem distância",
    description: "Mensagens, comentários e interações que transformam audiência em relacionamento.",
  },
  {
    icon: Store,
    title: "Marketplace integrado",
    description: "Venda produtos digitais, serviços e conteúdos sem tirar sua comunidade da plataforma.",
  },
  {
    icon: CircleDollarSign,
    title: "Monetização para criadores",
    description: "Assinaturas, conteúdos exclusivos e novas formas de gerar receita com sua presença digital.",
  },
  {
    icon: ShieldCheck,
    title: "Ambiente confiável",
    description: "Privacidade, moderação e segurança desde a base para uma rede saudável.",
  },
];
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-background/75 backdrop-blur-2xl">
        <div className="container flex h-16 items-center justify-between gap-6">
          ...
          <nav className="hidden items-center gap-7 text-sm text-muted-foreground md:flex">
            ...
            <a href="#experiencia" className="transition-colors hover:text-foreground">Experiência</a>
            <a href="#comecar" className="transition-colors hover:text-foreground">Começar</a>
          </nav>
      ...
      <main>
        <section className="container grid min-h-[calc(100dvh-4rem)] items-center gap-14 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:py-24">
            ...
            <FadeIn delay={0.15}>
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Button asChild size="lg" className="min-w-48 shadow-lg shadow-primary/20">
                  <Link href={primaryHref}>{primaryLabel}<ArrowRight /></Link>
                </Button>
                <Button asChild size="lg" variant="outline" className="min-w-44 bg-background/40">
                  <a href="#recursos"><Play className="h-4 w-4" />Conhecer a Lukave</a>
                </Button>
              </div>
            </FadeIn>
          ...
          <FadeIn delay={0.12} className="relative mx-auto w-full max-w-xl">
            ...
            <div className="glass-strong relative overflow-hidden rounded-[2rem] border-white/10 p-5 shadow-2xl shadow-black/30">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-sm font-semibold">Descobrir</p><p className="text-xs text-muted-foreground">Seu feed na Lukave</p></div>
                </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5"><p className="truncate text-sm font-semibold">Manson</p><BadgeCheck className="h-4 w-4 text-primary" /></div>
                    <p className="text-xs text-muted-foreground">@mansonware · agora</p>
                  </div>
                </div>
                <p className="mt-4 text-sm leading-6 text-foreground/90">Construindo experiências digitais que conectam pessoas, ideias e oportunidades. Esse é só o começo. ✦</p>
                <div className="mt-4 aspect-[16/9] rounded-xl border border-white/10 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.5),transparent_35%),linear-gradient(135deg,hsl(var(--background)),hsl(var(--muted)))] p-5">
                  <div className="flex h-full flex-col justify-between rounded-lg border border-white/10 bg-black/20 p-5 backdrop-blur">
                    ...
                    <div><p className="font-display text-2xl font-bold">Ideias ganham força quando encontram comunidade.</p><p className="mt-2 text-xs text-muted-foreground">lukave.com</p></div>
                  </div>
                </div>
        ...
        <section id="recursos" className="container py-20 sm:py-28">
          <FadeIn className="mx-auto mb-14 max-w-3xl text-center">
            ...
            <h2 className="font-display text-3xl font-bold tracking-tight sm:text-5xl">Tudo o que uma comunidade digital precisa.</h2>
            <p className="mx-auto mt-4 max-w-2xl text-muted-foreground sm:text-lg">Uma base social completa, preparada para crescer com criadores, marcas e comunidades.</p>
          </FadeIn>
            ...
            {features.map((feature, index) => (
              <FadeIn key={feature.title} delay={index * 0.04}>
                <article className="glass group h-full rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:bg-white/[0.04]">
                  <div className="mb-5 grid h-11 w-11 place-items-center rounded-xl border border-primary/20 bg-primary/10 text-primary transition-transform group-hover:scale-105"><feature.icon className="h-5 w-5" /></div>
        ...
        <section id="experiencia" className="container py-20 sm:py-28">
          <FadeIn>
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.025] p-7 text-center sm:p-14">
              <Badge variant="outline" className="mb-5">Feita para pessoas, não métricas</Badge>
              <h2 className="mx-auto max-w-3xl font-display text-3xl font-bold tracking-tight sm:text-5xl">Simples para entrar. Poderosa para crescer.</h2>
              <p className="mx-auto mt-5 max-w-2xl leading-7 text-muted-foreground">A Lukave reduz atrito para publicar, descobrir conteúdo e participar de comunidades. A experiência prioriza velocidade, clareza e conexão real.</p>
              <div className="mx-auto mt-9 grid max-w-4xl gap-4 sm:grid-cols-3">
                {[{ icon: Users, title: "Comunidade", text: "Encontre pessoas e construa conexões." }, { icon: Store, title: "Negócios", text: "Transforme presença em oportunidade." }, { icon: Zap, title: "Evolução", text: "Novos recursos sem perder simplicidade." }].map((item) => (
                  <div key={item.title} className="rounded-2xl border border-white/10 bg-background/60 p-6 text-left">
                    <item.icon className="h-6 w-6 text-primary" /><h3 className="mt-6 font-display text-xl font-semibold">{item.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{item.text}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <FadeIn>
            <div className="relative overflow-hidden rounded-[2rem] border border-primary/20 bg-primary/10 px-6 py-14 text-center sm:px-12 sm:py-20">
              ...
              <div className="relative mx-auto max-w-3xl">
                ...
                <h2 className="font-display text-3xl font-bold tracking-tight sm:text-5xl">A próxima conexão pode começar agora.</h2>
                <p className="mx-auto mt-5 max-w-2xl text-muted-foreground sm:text-lg">Crie seu perfil, descubra novas comunidades e acompanhe a evolução da Lukave desde os primeiros capítulos.</p>
                <Button asChild size="lg" className="mt-8 min-w-52 shadow-lg shadow-primary/25"><Link href={primaryHref}>{primaryLabel}<ArrowRight /></Link></Button>
              </div>
            </div>
          </FadeIn>
        </section>
